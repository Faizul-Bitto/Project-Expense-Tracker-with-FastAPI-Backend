...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      logger.info('Auth', `📧 Password reset OTP requested | Email=${email}`)
      navigate('/verify-otp', {
        state: { email, toastMessage: 'A password reset OTP has been sent. Please also check your spam/junk folder.', toastType: 'success' },
      })
    } catch (err) {
      const msg = err.response?.data?.detail || 'Something went wrong.'
      setError(msg)
      logger.error('Auth', `❌ Forgot password failed | Email=${email} | Reason=${msg}`)
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <Card className="border-0 bg-slate-900/80 backdrop-blur-sm shadow-2xl ring-1 ring-white/10">
          <CardContent className="pt-6">
            {error && <div className="bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-3 text-sm text-red-400 flex items-center gap-2 mb-5"><div className="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" />{error}</div>}
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Button type="submit" disabled={loading} className="w-full h-10 bg-linear-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white shadow-lg shadow-purple-500/20 font-medium">
                <Send className="w-4 h-4 mr-2" />{loading ? 'Sending...' : 'Send OTP'}
              </Button>
            </form>
