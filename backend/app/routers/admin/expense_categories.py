...
async def create_expense_category(
    ...
    if existing_expense_category:
        logger.warning(
            ...
            f"Name={create_expense_category_request.name} "
            f"already exists."
        )
...
async def update_expense_category(
    ...
    Only administrators are allowed to update
    expense categories.

    Raises:
        HTTPException:
            If the expense category does not exist
            or another category already uses the
            requested name.
    """
    ...
    existing_expense_category = (
        db.query(ExpenseCategory)
        .filter(
            ExpenseCategory.name == update_expense_category_request.name,
        )
        .filter(
            ExpenseCategory.id != category_id,
        )
    ...
    if existing_expense_category:
        logger.warning(
            ...
            f"Name={update_expense_category_request.name} "
            f"already exists."
        )
